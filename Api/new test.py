...
def Main(container_Dim, dataset, userID):
    ...
    result_array = []
    if len(objects) > 10:
        selected_index = [random.randint(0, len(objects)-1) for _ in range(random.randint(10, min(100,len(objects))))]
...
async def async_main(shapeArray, dataList, userId):
    loop = asyncio.get_event_loop()
    result = await loop.run_in_executor(None, Main, shapeArray, dataList, userId)
    return result
...
async def optimize():
    try:
        ...
        result = await async_main(shapeArray, dataList, str(userId))
